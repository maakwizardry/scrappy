 ...
 * scrapeGoogleMaps.js
 * 
 * Automated scraper that loops through standard keywords and Indian cities,
 * scrolling through Google Maps results to extract names, ratings, addresses, and phones.
 */
...
// Load Indian cities
const LOCATIONS = JSON.parse(fs.readFileSync(path.join(__dirname, "locationsIndia.json"), "utf8"));
...
async function startScraping() {
  ...
  const progressFile = path.join(__dirname, "progressIndia.json");
  ...
  console.log("🚀 Google Maps Scraper Worker Started");
        ...
        const results = await page.evaluate(() => {
          ...
          return Array.from(items).map((el) => {
            ...
            for (let line of lines) {
              // Match 10 digit Indian numbers with optional prefixes/spaces
              // e.g. 0987654321, 98765 43210, 022 12345678
              ...
              // Address heuristic: contains a pin code or commas separating locations
              else if (line.includes(",") && /\d/.test(line)) {
                address = line;
              }
              
              if (line.toLowerCase().includes(".com") || line.toLowerCase().includes(".in") || line.toLowerCase().includes("website")) {
                 website = line;
              }
            }
