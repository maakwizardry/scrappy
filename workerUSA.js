...
// US border-state codes covered by locationsUSA.json / scrapeGoogleMapsUSA.js
...
async function initDB() {
  ...
  console.log("✅ Enrichment Worker (USA) DB connected");
}
...
// ─────────────────────────────────────────────────────────────────────────────
// PROCESS ONE BUSINESS — USA (border states) ONLY
// ─────────────────────────────────────────────────────────────────────────────
async function processNextBusiness() {
  ...
  console.log(`\n⏳ Enriching (USA): ${business.name} (${business.website || "No website"})`);
...
async function startWorker() {
  await initDB();
  console.log("🚀 Enrichment Worker (USA) Started");
  ...
  while (true) {
    try {
      ...
      if (result.status === "done") {
        console.log("💤 Queue empty (USA). Waiting 30s...");
