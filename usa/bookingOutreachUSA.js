...
// US border-state codes covered by locationsUSA.json / scrapeGoogleMapsUSA.js / workerUSA.js
const US_STATE_CODES = ['WA', 'ID', 'MT', 'ND', 'MN', 'MI', 'NY', 'VT', 'NH', 'ME'];
...
async function initDB() {
  ...
  console.log('✅ Booking Outreach (USA) DB connected');
}
...
/**
 * Process the next business for booking outreach — USA (border states) ONLY
 * @returns {Object} Processing result
 */
async function processNextBookingLead() {
  if (!db) await initDB();

  const locationConditions = US_STATE_CODES.map(() => 'b.location LIKE ?').join(' OR ');
  const locationParams = US_STATE_CODES.map(code => `%, ${code}`);

  // Query for qualified home cleaning service businesses in the 10 US border states ONLY
  ...
  if (rows.length === 0) {
    return {
      status: 'done',
      message: 'No USA businesses left to process for booking outreach'
    };
  }

  const business = rows[0];
  console.log(`\n📋 Processing booking outreach (USA): ${business.name} [${business.location}]`);
...
async function startWorker() {
  await initDB();
  console.log('🚀 Booking Outreach Worker (USA) Started');
  console.log(`   Scoped to states: ${US_STATE_CODES.join(', ')}`);
  ...
  while (true) {
    try {
      ...
      if (result.status === 'done') {
        console.log('⏸️  Queue empty (USA). Waiting 30 seconds...');
...
if (process.argv[2] === '--test') {
  (async () => {
    console.log('🧪 TEST MODE (USA): Processing single business\n');
    await initDB();
    const result = await processNextBookingLead();
